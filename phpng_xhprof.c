 ...
 * @author kannan, hzhao
 */
static char *hp_opcode_get_function_name(const zend_execute_data const *execute_data TSRMLS_DC)
{
	const char *cls  = NULL;
	...
	//case ZEND_DO_FCALL_BY_NAME:
		{
			...
			case ZEND_USER_FUNCTION:
				...
				cls  = curr_func->common.scope ? curr_func->common.scope->name->val : (
						execute_data->called_scope ? execute_data->called_scope->name->val : NULL);
 ...
 * @author kannan, veeve
 */
int hp_hash_lookup(zval ** val, char *symbol TSRMLS_DC)
{
	HashTable *ht;
	zval * tmp;

	/* Bail if something is goofy */
	if (Z_TYPE_P(&hp_globals.stats_count) == IS_UNDEF || !(ht = HASH_OF(&hp_globals.stats_count)))
	{
		return -1;
	}
	...
	else
	{
		...
		array_init(*val);
		add_assoc_zval(&hp_globals.stats_count, symbol, *val);
	}
	return 0;
}
...
void hp_sample_stack(hp_entry_t **entries TSRMLS_DC)
{
	...
	/* Build key */
	snprintf(key, sizeof(key), "%d.%06d", hp_globals.last_sample_time.tv_sec,
...
void hp_sample_check(hp_entry_t **entries TSRMLS_DC)
{
	...
	while ((cycle_timer() - hp_globals.last_sample_tsc) > hp_globals.sampling_interval_tsc)
	{
		...
		incr_us_interval(&hp_globals.last_sample_time,
 ...
 * @author kannan
 */
int hp_mode_shared_endfn_cb(zval * val, hp_entry_t *top, char *symbol TSRMLS_DC)
{
	uint64 tsc_end;
	...
	/* Get the stat array */
	if (hp_hash_lookup(&val, symbol TSRMLS_CC) != 0)
	{
		return -1;
	}
